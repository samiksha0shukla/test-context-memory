...
export function MemoryGraph({ conversationId }: MemoryGraphProps) {
  ...
  // Debug logging
  useEffect(() => {
    console.log('MemoryGraph state:', { isLoading, hasError: !!error, hasData: !!data, nodeCount: data?.nodes?.length });
    if (error) {
  ...
  useEffect(() => {
    ...
    // Calculate and apply initial zoom to fit all bubbles
    ...
    // Apply zoom quickly after initial positions settle (100ms)
    setTimeout(calculateAndApplyZoom, 100);

    // Stop simulation after brief layout (1.5 seconds instead of 2)
    setTimeout(() => {
      simulation.stop();
